...
export default function DonationDetails({ admin, row, onClose, onSaved }) {
  ...
  const canFinance=adminCan(admin,"donations_manage");
  ...
  const editDocument=async(document)=>{const label=await requestText({title:"Edit donation document",label:"Document label",defaultValue:document.display_name||document.original_filename||"",submitLabel:"Next",required:true,trim:true});if(label===null||!label.trim())return;const type=await requestChoice({title:"Document type",label:"Type",options:DOC_TYPES.map(v=>({value:v,label:v})),defaultValue:document.document_type||"Other",submitLabel:"Save document"});if(type===null)return;const normalized=DOC_TYPES.find(v=>v===type);if(!normalized)return setError(`Choose: ${DOC_TYPES.join(", ")}.`);setDocBusy(true);try{await api.donations.updateDocument(row.id,document.id,{display_name:label.trim(),document_type:normalized});await loadDocuments();}catch(e){setError(e.message||"Could not update document");}finally{setDocBusy(false);}};
  const removeDocument=async(document)=>{const reason=await requestText({title:"Remove donation document",label:"Removal reason",submitLabel:"Remove document",required:true,minLength:3,multiline:true,trim:true});if(!reason)return;setDocBusy(true);try{await api.donations.removeDocument(row.id,document.id,reason);await loadDocuments();await refresh();}catch(e){setError(e.message||"Could not remove document");}finally{setDocBusy(false);}};
  const reverse=async()=>{const reason=await requestText({title:"Reverse donation",label:"Reversal reason",submitLabel:"Continue",required:true,minLength:3,multiline:true,trim:true});if(!reason)return;setBusy(true);setError("");try{const result=await api.governance.reverse("donation",row.id,reason);await onSaved(`Donation reversed · ${result.reversal_id}`);onClose();}catch(e){setError(e.message||"Could not reverse donation");}finally{setBusy(false);}};

  if(editing&&canFinance)return <DonationModal row={current} onClose={()=>setEditing(false)} onSaved={async(message)=>{await refresh();await onSaved(message||"Donation updated");setEditing(false);}}/>;
  const d=current||row;
  return <>
