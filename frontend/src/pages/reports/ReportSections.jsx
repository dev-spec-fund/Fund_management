...
export function AnnualAnalyticsSection({ annualYear, setAnnualYear, annual, analytics, annualBusy, loadAnnual }) {
  return <>
    ...
    <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: 14, marginBottom: 14 }}>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button type="button" disabled={annualBusy || annualYear.length !== 4} onClick={async () => { try { await loadAnnual(); } catch (e) { window.alert(e.message || "Could not load annual analytics"); } }} style={{ ...smallBtn("var(--primary-text)"), flex: 1 }}>{annualBusy ? "Loading…" : "Load annual report"}</button>
