...
export default function Reports({ setTab }) {
  ...
  const loadAnnual=async()=>{
    setAnnualBusy(true);
    try{const [a,x]=await Promise.all([api.governance.annual(annualYear),api.governance.analytics(annualYear)]);setAnnual(a);setAnalytics(x)}
    catch(e){window.alert(e.message||"Could not load annual analytics")}finally{setAnnualBusy(false)}
  };
