...
function detectedFileMime(bytes: ArrayBuffer, headerMime: string | null, filePath: string): string {
  const b = new Uint8Array(bytes, 0, Math.min(bytes.byteLength, 16));
  // Prefer the actual file signature. Telegram's file endpoint may respond with
  // application/octet-stream even when a bot photo is a normal JPEG.
  if (b.length >= 3 && b[0] === 0xff && b[1] === 0xd8 && b[2] === 0xff) return "image/jpeg";
  if (b.length >= 8 && b[0] === 0x89 && b[1] === 0x50 && b[2] === 0x4e && b[3] === 0x47 && b[4] === 0x0d && b[5] === 0x0a && b[6] === 0x1a && b[7] === 0x0a) return "image/png";
  if (b.length >= 12 && String.fromCharCode(...b.slice(0, 4)) === "RIFF" && String.fromCharCode(...b.slice(8, 12)) === "WEBP") return "image/webp";
  if (b.length >= 4 && b[0] === 0x25 && b[1] === 0x50 && b[2] === 0x44 && b[3] === 0x46) return "application/pdf";

  const normalized = String(headerMime || "").split(";")[0].trim().toLowerCase();
  if (normalized && normalized !== "application/octet-stream" && normalized !== "binary/octet-stream") return normalized;
...
/** Downloads a Telegram file (e.g. slip photo) and returns its bytes + normalized mime type. */
export async function downloadTelegramFile(env: Env, fileId: string): Promise<{ bytes: ArrayBuffer; mime: string } | null> {
  ...
  const mime = detectedFileMime(bytes, fileRes.headers.get("content-type"), filePath);
  return { bytes, mime };
}
