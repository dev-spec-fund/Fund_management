...
export const GRANULAR_ADMIN_PERMISSIONS = [
...
export type AdminPermissionKey = "read" | "finance" | typeof GRANULAR_ADMIN_PERMISSIONS[number];
const GRANULAR_PERMISSION_SET = new Set<string>(GRANULAR_ADMIN_PERMISSIONS as readonly string[]);

const BUILTIN_PERMISSION_MAP: Record<string, Set<string>> = {
  president: new Set([
...
export function adminCan(admin: Admin | null | undefined, permission: AdminPermissionKey) {
  ...
  if (admin.custom_role_id) {
    ...
    const hasGranular = permissions.some(p => GRANULAR_PERMISSION_SET.has(p));
    if (hasGranular) {
      if (permissions.includes(permission)) return true;
      // Manage permissions always include the matching read permission.
    ...
    // Backward compatibility for roles created before Role Builder v2.
    if (permissions.includes(permission)) return true;
...
export async function ensureOperationalSchema(env: Env) {
  ...
  try {
    ...
    const checks:[string,string[]][] = [
      ...
      ["election_applications", ["election_id","position_id","member_id","statement","status","submitted_at","reviewed_at","reviewed_by","review_reason","withdrawn_at"]],
      ["election_runoffs", ["election_id","position_id","round_no","seats_to_fill","status","opened_at","closed_at","created_by"]],
      ["election_runoff_candidates", ["runoff_id","candidate_id"]],
      ...
      ["exco_terms", ["election_id","term_label","status","started_at","ended_at"]],
      ["exco_handover_records", ["incoming_term_id","outgoing_term_id","status","notes","completed_at","completed_by"]],
