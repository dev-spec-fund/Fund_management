...
donationsRoute.post("/", requireAdmin, async (c) => {
  ...
  const txnId = await generateTxnId(c.env, "D");
  const res = await c.env.DB.prepare(
    "INSERT INTO donations (txn_id, donor_name, amount, note, logged_by) VALUES (?, ?, ?, ?, ?)"
  ).bind(txnId, body.donor_name, body.amount, body.note || null, admin.id).run();
  await logAudit(c.env, admin.id, "log_donation", `${txnId} — ${body.donor_name} — MVR ${body.amount}`);
