...
expensesRoute.post("/", requireFinance, async (c) => {
  ...
  const res=await c.env.DB.prepare(`INSERT INTO expenses
    (txn_id,description,category_id,amount,logged_by,expense_date,transaction_month,status,approval_required,approved_by,approved_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(txnId,description,category?.id ?? null,amount,admin.id,expenseDate,month,needsApproval?"pending":"approved",needsApproval?1:0,needsApproval?null:admin.id,needsApproval?null:new Date().toISOString()).run();
  await auditEntity(c.env,admin.id,"expense_created","expense",Number(res.meta.last_row_id),null,{txn_id:txnId,description,amount,expense_date:expenseDate,month,status:needsApproval?"pending":"approved"});
...
expensesRoute.patch("/:id", requireFinance, async (c) => {
  ...
  if(!description || amount===null) return c.json({error:"Description and valid positive amount are required"},400);
  const threshold=money(await getSetting(c.env,"expense_approval_threshold")) || 5000;
  const materialChanged = description!==before.description || Number(requestedCategory)!==Number(before.category_id) || Math.abs(amount-Number(before.amount))>0.004 || month!==originalMonth || String(expenseDate||'')!==String(before.expense_date||'');
  ...
  await c.env.DB.prepare("UPDATE expenses SET description=?,category_id=?,amount=?,expense_date=?,transaction_month=?,edited_by=?,updated_at=datetime('now'),status=?,approval_required=?,approved_by=?,approved_at=? WHERE id=?")
    .bind(description,requestedCategory,amount,expenseDate,month,admin.id,status,approvalRequired,approvedBy,approvedAt,id).run();
  const after=await c.env.DB.prepare("SELECT * FROM expenses WHERE id=?").bind(id).first<any>();
